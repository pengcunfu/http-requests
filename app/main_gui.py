"""
HTTP请求解析器GUI主界面 - 保持原有UI结构，使用默认样式
"""
import sys
import json
import time
...
from PySide6.QtWidgets import (
    ...
    QGridLayout, QSpinBox, QCheckBox, QDialog
)
...
class RawRequestDialog(QDialog):
    """原始HTTP请求导入对话框"""

    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("导入原始HTTP请求")
    ...
    def setup_ui(self):
        ...
        # 说明文本
        hint_label = QLabel(
            "粘贴原始HTTP请求文本（从浏览器开发者工具、Postman等复制）\n"
            "格式示例:\n"
    ...
    def parse_request(self):
        ...
        if not raw_request.strip():
            QMessageBox.warning(self, "输入错误", "请输入HTTP请求文本")
            return
        ...
        if not result['success']:
            QMessageBox.critical(self, "解析失败", result['error'])
        ...
        # 显示预览
        preview = []
        preview.append(f"方法: {result['method']}")
        preview.append(f"URL: {result['url']}")
        preview.append(f"路径: {result['path']}")
        preview.append(f"协议: {result['protocol']}")
        preview.append(f"\n请求头数量: {len(result['headers'])}")
        ...
        if result['headers']:
            ...
            if len(result['headers']) > 5:
                preview.append(f"  ... 还有 {len(result['headers']) - 5} 个请求头")

        if result['body']:
            preview.append(f"\n请求体长度: {len(result['body'])} 字符")
            body_preview = result['body'][:200]
            if len(result['body']) > 200:
                body_preview += "..."
            preview.append(f"请求体预览:\n{body_preview}")
        ...
        # 显示成功消息
        QMessageBox.information(self, "解析成功", "HTTP请求解析成功！点击\"导入\"按钮将数据填充到主界面。")
...
class HTTPClient(QMainWindow):
    ...
    def create_menu_bar(self):
        ...
        import_raw_action = tools_menu.addAction('导入原始请求')
        import_raw_action.triggered.connect(self.import_raw_request)
    ...
    def create_headers_tab(self):
        ...
        # 模式切换按钮
        self.toggle_headers_mode_btn = QPushButton("切换到文本模式")
        self.toggle_headers_mode_btn.clicked.connect(self.toggle_headers_mode)
        ...
        headers_buttons_layout.addWidget(self.toggle_headers_mode_btn)

        headers_layout.addLayout(headers_buttons_layout)

        # 创建表格模式容器
        self.headers_table_widget = QWidget()
        headers_table_layout = QVBoxLayout(self.headers_table_widget)
        headers_table_layout.setContentsMargins(0, 0, 0, 0)
        ...
        headers_table_layout.addWidget(self.headers_table)

        # 创建文本模式容器
        self.headers_text_widget = QWidget()
        headers_text_layout = QVBoxLayout(self.headers_text_widget)
        headers_text_layout.setContentsMargins(0, 0, 0, 0)

        # 文本模式说明
        text_hint = QLabel("每行一个请求头，格式：Key: Value\n以 # 开头的行将被忽略")
        headers_text_layout.addWidget(text_hint)

        # 请求头文本编辑器
        self.headers_text = QTextEdit()
        self.headers_text.setPlaceholderText("Content-Type: application/json\nAuthorization: Bearer token123\n# User-Agent: Custom Agent")
        headers_text_layout.addWidget(self.headers_text)

        # 默认显示表格模式
        headers_layout.addWidget(self.headers_table_widget)
        headers_layout.addWidget(self.headers_text_widget)
        self.headers_text_widget.setVisible(False)
        self.headers_mode = "table"  # "table" 或 "text"
        ...
        if self.headers_mode == "table":
            ...
            for row in range(self.headers_table.rowCount()):
                ...
                if key_item and value_item:
                    ...
                    if key and value:
                        # 如果未启用，添加注释符号
                        prefix = "" if (checkbox and checkbox.isChecked()) else "# "
                        headers_text_lines.append(f"{prefix}{key}: {value}")

            self.headers_text.setPlainText("\n".join(headers_text_lines))

            # 切换显示
            self.headers_table_widget.setVisible(False)
            self.headers_text_widget.setVisible(True)
            self.toggle_headers_mode_btn.setText("切换到表格模式")
            self.headers_mode = "text"
        else:
            # 从文本模式切换到表格模式
            # 先将文本数据转换为表格
            text = self.headers_text.toPlainText()
            self.parse_headers_text_to_table(text)

            # 切换显示
            self.headers_text_widget.setVisible(False)
            self.headers_table_widget.setVisible(True)
            self.toggle_headers_mode_btn.setText("切换到文本模式")
            self.headers_mode = "table"

    def parse_headers_text_to_table(self, text):
        """将文本格式的请求头解析到表格"""
    ...
    def import_raw_request(self):
        """导入原始HTTP请求"""
        dialog = RawRequestDialog(self)
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            if parsed_data and parsed_data['success']:
                # 填充方法
                self.method_combo.setCurrentText(parsed_data['method'])

                # 填充URL
                self.url_input.setText(parsed_data['url'])

                # 填充请求头
                self.load_headers(parsed_data['headers'])

                # 填充请求体
                if parsed_data['body']:
                    self.body_edit.setPlainText(parsed_data['body'])
                else:
                    self.body_edit.clear()

                # 显示成功消息
                self.status_bar.showMessage(
                    f"成功导入 {parsed_data['method']} 请求: {parsed_data['url']}", 5000
                )
...
# 为了向后兼容，保留原始类名
HTTPRequestParserGUI = HTTPClient